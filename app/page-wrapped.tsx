...
import { Upload, Link2, Copy, Download, FileText, ImageIcon, Code, Clock, Zap, Shield, Send } from "lucide-react";
...
function HomeContent() {
  ...
  const [textAccessCode, setTextAccessCode] = useState("");
  ...
  const handleRetrieveText = async () => {
    if (!textAccessCode.trim()) return;
    
    // Simply redirect to the text viewing page
    window.location.href = `/t/${textAccessCode.toUpperCase()}`;
  };
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-muted/20 rounded-lg p-4 max-w-sm mx-auto">
                          ...
                          <div className="text-center">
                            ...
                            <div className="inline-block p-2 bg-white/95 rounded-lg">
                              <QRCodeGenerator 
              ...
              <Card className="glass-strong glow-soft">
                ...
                <CardContent className="space-y-6 px-8 pb-8">
                  <Tabs defaultValue="send" className="w-full">
                    <TabsList className="grid w-full grid-cols-2 glass h-10 mb-6">
                      <TabsTrigger value="send" className="flex items-center gap-2 data-[state=active]:bg-primary/20">
                        <Send className="w-4 h-4" />
                        Send
                      </TabsTrigger>
                      <TabsTrigger value="retrieve" className="flex items-center gap-2 data-[state=active]:bg-primary/20">
                        <Download className="w-4 h-4" />
                        Retrieve
                      </TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="send" className="space-y-4">
                      ...
                      <div className="flex gap-3">
                        ...
                        >
                          ...
                          {textUploading ? "Sending..." : "Send Text"}
                        </Button>
                        <Button 
                          variant="outline" 
                          size="lg" 
                      ...
                      {textShareCode && (
                        <div className="bg-muted/10 rounded-lg p-4 border border-primary/20 glow-soft">
                          <div className="flex items-center justify-between mb-3">
                            <span className="text-sm font-medium text-primary">✨ Text Sent Successfully!</span>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="bg-card/20 rounded p-3">
                              <p className="text-xs text-muted-foreground mb-1">Text Preview:</p>
                              <div className="text-sm font-mono bg-muted/20 rounded p-2 max-h-20 overflow-y-auto text-muted-foreground">
                            ...
                            <div className="bg-card/20 rounded p-3 text-center">
                              ...
                              <div className="inline-block p-2 bg-white/95 rounded">
                                <QRCodeGenerator 
                    ...
                    <TabsContent value="retrieve" className="space-y-4">
                      <div className="space-y-3">
                        ...
                        <Input 
                          ...
                          className="glass text-center text-2xl tracking-[0.3em] uppercase font-mono h-14"
                          maxLength={6}
                          value={textAccessCode}
                          onChange={(e) => setTextAccessCode(e.target.value.toUpperCase())}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' && textAccessCode.trim()) {
                              handleRetrieveText();
                            }
                          }}
                        />
                        <p className="text-xs text-muted-foreground text-center">
                          Press Enter or click View Text to see the shared content
                        </p>
                      </div>
                      
                      <Button 
                        className="w-full h-12 text-base hover-glow" 
                        onClick={handleRetrieveText} 
                        disabled={!textAccessCode.trim()}
                      >
                        <Download className="w-5 h-5 mr-3" />
                        View Text
                      </Button>
                      ...
                      <div className="bg-muted/10 rounded-lg p-4 border border-border/20">
                        <div className="text-center space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Enter the 6-digit code and you&apos;ll be taken to a page where you can view and copy the shared text.
                          </p>
                        </div>
                      </div>
                    </TabsContent>
